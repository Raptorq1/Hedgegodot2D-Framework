...
func _middle_enter(host, prev_state, state, state_name:String) -> void:
	...
	state_collection.get_state(state_name).connect("finished", host.fsm, "change_state", [], CONNECT_ONESHOT)
...
func _middle_exit(host, next_state, state, state_name:String) -> void:
	...
	state_collection.get_state(state_name).disconnect("finished", host.fsm, "change_state")
